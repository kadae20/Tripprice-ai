-- ──────────────────────────────────────────────────────────────
-- Tripprice AI — Supabase DB 스키마 (수정본)
-- ──────────────────────────────────────────────────────────────
...
CREATE INDEX idx_decision_sessions_session_id
  ON decision_sessions(session_id);
CREATE INDEX idx_decision_sessions_city
  ON decision_sessions(city);
CREATE INDEX idx_decision_sessions_travel_purpose
  ON decision_sessions(travel_purpose);

-- 호텔별 전환율 집계 뷰 (수정: selected_hotel_id → hotel_id)
CREATE VIEW hotel_conversion_stats AS
SELECT
